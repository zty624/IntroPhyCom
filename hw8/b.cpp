...
int mortecarlo(IsingLattice& lattice, cords_t boxsize, double beta)
{
    ...
    auto& particle = dynamic_cast<IsingParticle&>(*lattice.particles[particle_idx]);
    const int delta_energy = 2 * lattice.getLocalEnergy(particle_idx);
    if (delta_energy < 0 && uniform_real() > exp(beta * delta_energy)) {
        return 1; // Reject the move
    }
    particle.spin = static_cast<IsingSpin>(-particle.spin);
    return 0;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 4) {
        ...
        cout << "Usage: " << argv[0] << " <boxsize> <num_ensembles> <total_steps>\n";
        cin >> boxsize >> num_ensembles >> total_steps;
    }

    cords_t dimensions{boxsize, boxsize};
    // beta = 0.5 \ln (1 + \sqrt{2})
    const double beta = 0.5 * log(1 + sqrt(2));
    const int sample_interval = boxsize * boxsize;
